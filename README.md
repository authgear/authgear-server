...
The dev server of parcel@2.0.0-rc.0 enables Cross-Origin-Embedder-Policy by default.
That would prevent from us from loading `picture` from origins that are not yet Cross-Origin-Resource-Policy aware.
So a `.proxyrc.js` was added to remove the Cross-Origin-Embedder-Policy header.
That change was reverted https://github.com/parcel-bundler/parcel/issues/6561 so we should remove our workaround when we upgrade parcel.
