...
// ACLEntry grants access to a record by relation or by user_id
...
const (
	...
	WriteLevel  ACLLevel = "write"
	CreateLevel ACLLevel = "create"
)

// NewACLEntryRelation returns an ACE on relation
func NewACLEntryRelation(relation string, level ACLLevel) ACLEntry {
	return ACLEntry{
		Relation: relation,
		Level:    level,
	}
...
// NewACLEntryDirect returns an ACE for a specific user
func NewACLEntryDirect(userID string, level ACLLevel) ACLEntry {
	return ACLEntry{
		Relation: "$direct",
		Level:    level,
		UserID:   userID,
...
// NewACLEntryRole return an ACE on role
func NewACLEntryRole(role string, level ACLLevel) ACLEntry {
	return ACLEntry{
		Role:  role,
		Level: level,
	}
...
// NewACLEntryPublic return an ACE on public access
func NewACLEntryPublic(level ACLLevel) ACLEntry {
	return ACLEntry{
		Public: true,
		Level:  level,
	}
...
func (ace *ACLEntry) Accessible(authinfo *authinfo.AuthInfo, level ACLLevel) bool {
	if ace.Public {
...
func (ace *ACLEntry) AccessibleLevel(level ACLLevel) bool {
	if level == ReadLevel {
		return true
	}
...
// ACL is a list of ACL entries defining access control for a record
type ACL []ACLEntry
...
func (acl ACL) Accessible(authinfo *authinfo.AuthInfo, level ACLLevel) bool {
	if len(acl) == 0 {
...
func (r FieldUserRole) Match(authinfo *authinfo.AuthInfo, record *Record) bool {
...
// AuthInfo and Record.
func (r FieldUserRole) matchDynamic(authInfo *authinfo.AuthInfo, record *Record) bool {
