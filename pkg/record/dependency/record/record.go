...
// ID identifies an unique record in a Database
type ID struct {
	Type string
	Key  string
}

// NewRecordID returns a new ID
func NewRecordID(recordType string, id string) ID {
	return ID{recordType, id}
}

func NewEmptyRecordID() ID {
	return ID{"", ""}
}
...
// IsEmpty returns whether the ID is empty.
...
func (r *Record) Accessible(authinfo *authinfo.AuthInfo, level ACLLevel) bool {
	if r.ACL == nil {
		return true
	}
	userID := ""
...
// Schema is a mapping of record key to its value's data type or reference
type Schema map[string]FieldType

// DefinitionCompatibleTo returns if a record having the specified Schema
//
// can be saved to a database table of this Schema.
//
// This function is not associative. In other words, `a.fn(b) != b.fn(a)`.
func (schema Schema) DefinitionCompatibleTo(other Schema) bool {
...
func (schema Schema) HasField(field string) bool {
	_, found := schema[field]
	return found
}

func (schema Schema) HasFields(fields []string) bool {
...
// FieldType represents the kind of data living within a field of a Schema.
