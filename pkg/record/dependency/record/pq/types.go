...
func pqDataType(dataType record.DataType) string {
	switch dataType {
	default:
		panic(fmt.Sprintf("Unsupported dataType = %s", dataType))
	case record.TypeString, record.TypeAsset, record.TypeReference:
		return TypeString
...
type referenceValue record.Reference
