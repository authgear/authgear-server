...
type DCRCommands interface {
	...
	DeleteClient(ctx context.Context, clientID string) (*model.OAuthClient, error)
}

type DCRQueries interface {
	ListInitialAccessTokens(ctx context.Context) ([]*model.OAuthInitialAccessToken, error)
	ListClients(ctx context.Context, pageArgs graphqlutil.PageArgs, source *model.OAuthClientSource, clientIDs []string) (*oauthclient.ListClientResult, error)
}
...
func (f *DCRFacade) DeleteClient(ctx context.Context, clientID string) (*model.OAuthClient, error) {
	return f.DCRCommands.DeleteClient(ctx, clientID)
}

func (f *DCRFacade) ListClients(ctx context.Context, pageArgs graphqlutil.PageArgs, source *model.OAuthClientSource, clientIDs []string) ([]model.PageItemRef, *graphqlutil.PageResult, error) {
	result, err := f.DCRQueries.ListClients(ctx, pageArgs, source, clientIDs)
	if err != nil {
		return nil, nil, err
	}
