	...
	return []interaction.Effect{
		...
		interaction.EffectOnCommit(func(goCtx context.Context, ctx *interaction.Context, graph *interaction.Graph, nodeIndex int) error {
			...
			if !n.UpdateLoginTime.IsZero() {
				...
				logger.WithSkipStackTrace().Debug(goCtx, "updated last login",
			...
			if n.SessionToCreate != nil {
				if n.CreateReason == session.CreateReasonLogin || n.CreateReason == session.CreateReasonReauthenticate {
					...
					// NOTE(DEV-2982): This is for debugging the session lost problem
					logger := doEnsureSessionLogger.GetLogger(goCtx)
					logger.WithSkipStackTrace().Debug(goCtx, "user.authenticated event skipped because create reason is not login or reauthenticate",
						slog.String("user_id", userID),
				...
				// NOTE(DEV-2982): This is for debugging the session lost problem
				logger := doEnsureSessionLogger.GetLogger(goCtx)
				logger.WithSkipStackTrace().Debug(goCtx, "user.authenticated event skipped because session to create is nil",
					slog.String("user_id", userID),
					slog.Bool("refresh_token_log", true))
			}
