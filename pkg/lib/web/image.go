...
var localeAwareStaticImageRegex = regexp.MustCompile(`^static/([a-zA-Z0-9-]+)/(.+)\.(png|jpe|jpeg|jpg|gif)$`)

type LocaleAwareStaticImageDescriptor struct {
	Name string
}

var _ resource.Descriptor = LocaleAwareStaticImageDescriptor{}

func (a LocaleAwareStaticImageDescriptor) MatchResource(path string) (*resource.Match, bool) {
	matches := localeAwareStaticImageRegex.FindStringSubmatch(path)
	if len(matches) != 4 {
		return nil, false
	}
	languageTag := matches[1]
...
func (a LocaleAwareStaticImageDescriptor) FindResources(fs resource.Fs) ([]resource.Location, error) {
	if fs.GetFsLevel() != resource.FsLevelBuiltin {
		return []resource.Location{}, nil
	}
...
func (a LocaleAwareStaticImageDescriptor) ViewResources(resources []resource.ResourceFile, rawView resource.View) (interface{}, error) {
	switch view := rawView.(type) {
	case resource.AppFileView:
		return a.viewAppFile(resources, view)
	case resource.EffectiveFileView:
...
func (a LocaleAwareStaticImageDescriptor) viewByPath(resources []resource.ResourceFile, path string) (*StaticAsset, error) {
	matches := localeAwareStaticImageRegex.FindStringSubmatch(path)
	if len(matches) < 4 {
		return nil, resource.ErrResourceNotFound
	}
	...
	var bytes []byte
	for _, resrc := range resources {
		m := localeAwareStaticImageRegex.FindStringSubmatch(resrc.Location.Path)
		langTag := m[1]
		extension := m[3]
		if langTag == requestedLangTag && extension == requestedExtension {
			found = true
...
func (a LocaleAwareStaticImageDescriptor) viewAppFile(resources []resource.ResourceFile, view resource.AppFileView) (interface{}, error) {
	return nil, nil
}

func (a LocaleAwareStaticImageDescriptor) viewEffectiveFile(resources []resource.ResourceFile, view resource.EffectiveFileView) (interface{}, error) {
	path := view.EffectiveFilePath()
	asset, err := a.viewByPath(resources, path)
	if err != nil {
		return nil, err
...
func (a LocaleAwareStaticImageDescriptor) viewEffectiveResource(resources []resource.ResourceFile, view resource.EffectiveResourceView) (interface{}, error) {
	preferredLanguageTags := view.PreferredLanguageTags()
	defaultLanguageTag := view.DefaultLanguageTag()
	...
	extractLanguageTag := func(resrc resource.ResourceFile) string {
		langTag := localeAwareStaticImageRegex.FindStringSubmatch(resrc.Location.Path)[1]
		return langTag
	}
	add := func(langTag string, resrc resource.ResourceFile) error {
		if resrc.Location.Fs.GetFsLevel() == resource.FsLevelBuiltin {
...
func (a LocaleAwareStaticImageDescriptor) viewValidateResource(resources []resource.ResourceFile, view resource.ValidateResourceView) (interface{}, error) {
	return nil, nil
}

func (a LocaleAwareStaticImageDescriptor) UpdateResource(_ context.Context, _ []resource.ResourceFile, resrc *resource.ResourceFile, data []byte) (*resource.ResourceFile, error) {
	return nil, fmt.Errorf("Static image resource cannot be updated. Use locale aware image or non locale aware image resource instead.")
}
