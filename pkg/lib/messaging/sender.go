	...
	sendInTx := func(ctx context.Context) error {
		...
		if err != nil {
			...
			if errors.As(err, &smsapiErr) {
				if smsapiErr.IsNonCritical {
					// Skip sentry logging if it is marked as non-critical by the sms gateway
					errorLogger = errorLogger.WithSkipLogging()
				}
			}
			errorLogger.WithError(err).With(
...
func (s *Sender) SendWhatsappInNewGoroutine(ctx context.Context, msgType translation.MessageType, opts *whatsapp.SendAuthenticationOTPOptions, callback SendWhatsappResultCallback) error {
	...
	if s.DevMode {
		result, err := s.devModeSendWhatsapp(ctx, msgType, opts)
		if err != nil {
			return err
		}
		callback(ctx, result)
