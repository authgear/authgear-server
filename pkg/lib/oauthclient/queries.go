...
func (q *Queries) ListClients(ctx context.Context, pageArgs graphqlutil.PageArgs, source *model.OAuthClientSource, clientIDs []string) (*ListClientResult, error) {
	storeResult, err := q.Store.ListClients(ctx, pageArgs, source, clientIDs)
	if err != nil {
		return nil, err
	}
