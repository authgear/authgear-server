...
func (c *EmailChecker) Validate(ctx context.Context, validationCtx *validation.Context, loginID string) {
	...
	if c.BlockFreeEmailProviderDomains != nil {
		...
		if matched {
			validationCtx.EmitError("blocked", map[string]interface{}{"reason": "EmailFreeDomainBlocklist"})
			return
		}
	}
	...
	if c.BlockDisposableEmailDomains != nil {
		...
		if matched {
			validationCtx.EmitError("blocked", map[string]interface{}{"reason": "EmailDisposableDomainBlocklist"})
			return
		}
	}
