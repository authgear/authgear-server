...
func (s *Store) DeleteUser(appID string, userID string) error {
...
func (s *Store) CleanupUsers(appID string, keepUserIDs []string) (int64, error) {
	q := s.SQLBuilder.WithAppID(appID).
		Delete(s.SQLBuilder.TableName("_search_user")).
		Where("NOT id = ANY (?)", pq.Array(keepUserIDs))

	r, err := s.SQLExecutor.ExecWith(q)
	if err != nil {
		return 0, err
	}
...
func (s *Store) UpsertUsers(users []*model.SearchUserSource) error {
