...
func TestAuthorizationHandler(t *testing.T) {
	...
	Convey("Authorization handler", t, func() {
		...
		Convey("authflow", func() {
			Convey("allow listed flow group", func() {
				h.UIConfig = &config.UIConfig{
				...
				resp := handle(protocol.AuthorizationRequest{
					...
					"response_type":               "none",
					"x_authentication_flow_group": "group-1",
				})
				So(resp.Result().StatusCode, ShouldEqual, 200)
			})

			Convey("disallow undefined flow group", func() {
				h.UIConfig = &config.UIConfig{
					Implementation:     config.UIImplementationAuthflowV2,
					AuthenticationFlow: config.UIAuthenticationFlowConfig{},
				}
				...
				resp := handle(protocol.AuthorizationRequest{
					...
					"response_type":               "none",
					"scope":                       "openid",
					"x_authentication_flow_group": "group-2",
				})
				So(resp.Result().StatusCode, ShouldEqual, 200)
				So(resp.Body.String(), ShouldEqual, redirectHTML(
					"https://example.com/?error=invalid_request&error_description=authentication+flow+group+is+not+defined",
				))
			})

			Convey("disallow unlisted flow group", func() {
				h.UIConfig = &config.UIConfig{
					Implementation: config.UIImplementationAuthflowV2,
					AuthenticationFlow: config.UIAuthenticationFlowConfig{
						Groups: []config.UIAuthenticationFlowGroup{
				...
				So(resp.Result().StatusCode, ShouldEqual, 200)
				So(resp.Body.String(), ShouldEqual, redirectHTML(
					"https://example.com/?error=invalid_request&error_description=authentication+flow+group+is+not+allowed",
				))
			})

			Convey("disallow unlisted flow group with allowlist", func() {
				h.UIConfig = &config.UIConfig{
					Implementation: config.UIImplementationAuthflowV2,
					AuthenticationFlow: config.UIAuthenticationFlowConfig{
						Groups: []config.UIAuthenticationFlowGroup{
				...
				resp := handle(protocol.AuthorizationRequest{
					...
					"response_type":               "none",
					"scope":                       "openid",
					"x_authentication_flow_group": "group-3",
				})
				So(resp.Result().StatusCode, ShouldEqual, 200)
				So(resp.Body.String(), ShouldEqual, redirectHTML(
					"https://example.com/?error=invalid_request&error_description=authentication+flow+group+is+not+defined",
