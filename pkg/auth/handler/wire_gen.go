...
import (
	...
	pq4 "github.com/skygeario/skygear-server/pkg/auth/dependency/mfa/pq"
...
func newSignupHandler(r *http.Request, m auth.DependencyMap) http.Handler {
	...
	oauthProvider := oauth3.ProvideOAuthProvider(sqlBuilder, sqlExecutor)
	...
	mfaStore := pq4.ProvideStore(tenantConfiguration, sqlBuilder, sqlExecutor, provider)
...
func newLogoutHandler(r *http.Request, m auth.DependencyMap) http.Handler {
	...
	passwordhistoryStore := pq.ProvidePasswordHistoryStore(provider, sqlBuilder, sqlExecutor)
	reservedNameChecker := auth.ProvideReservedNameChecker(m)
	passwordProvider := password2.ProvidePasswordProvider(sqlBuilder, sqlExecutor, provider, passwordhistoryStore, factory, tenantConfiguration, reservedNameChecker)
