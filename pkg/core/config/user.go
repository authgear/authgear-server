package config

import (
	"net/url"
)

// UserConfiguration represents user-editable configuration
type UserConfiguration struct {
	Version          string                         `json:"version"`
	APIKey           string                         `json:"api_key"`
	MasterKey        string                         `json:"master_key"`
	URLPrefix        string                         `json:"url_prefix"`
	CORS             CORSConfiguration              `json:"cors"`
	Auth             NewAuthConfiguration           `json:"auth"`
	TokenStore       NewTokenStoreConfiguration     `json:"token_store"`
	UserAudit        NewUserAuditConfiguration      `json:"user_audit"`
	ForgotPassword   NewForgotPasswordConfiguration `json:"forgot_password"`
	WelcomeEmail     NewWelcomeEmailConfiguration   `json:"welcome_email"`
	SSO              NewSSOConfiguration            `json:"sso"`
	UserVerification UserVerificationConfiguration  `json:"user_verification"`
}

// CORSConfiguration represents CORS configuration.
// Currently we only support configuring origin.
// We may allow to support other headers in the future.
// The interpretation of origin is done by this library
// https://github.com/iawaknahc/originmatcher
type CORSConfiguration struct {
	Origin string `json:"origin"`
}

type NewAuthConfiguration struct {
	LoginIDKeys       []string `json:"login_id_keys"`
	CustomTokenSecret string   `json:"custom_token_secret"`
}

type NewTokenStoreConfiguration struct {
	Secret string `json:"secret"`
	Expiry int64  `json:"expiry"`
}

type NewUserAuditConfiguration struct {
	Enabled         bool                  `json:"enabled"`
	TrailHandlerURL string                `json:"trail_handler_url"`
	Password        PasswordConfiguration `json:"password"`
}

type PasswordConfiguration struct {
	MinLength             int      `json:"min_length"`
	UppercaseRequired     bool     `json:"uppercase_required"`
	LowercaseRequired     bool     `json:"lowercase_required"`
	DigitRequired         bool     `json:"digit_required"`
	SymbolRequired        bool     `json:"symbol_required"`
	MinimumGuessableLevel int      `json:"minimum_guessable_level"`
	ExcludedKeywords      []string `json:"excluded_keywords"`
	// Do not know how to support fields because we do not
	// have them now
	// ExcludedFields     []string `json:"excluded_fields"`
	HistorySize int `json:"history_size"`
	HistoryDays int `json:"history_days"`
	ExpiryDays  int `json:"expiry_days"`
}

type NewForgotPasswordConfiguration struct {
	AppName             string `json:"app_name"`
	URLPrefix           string `json:"url_prefix"`
	SecureMatch         bool   `json:"secure_match"`
	SenderName          string `json:"sender_name"`
	Sender              string `json:"sender"`
	Subject             string `json:"subject"`
	ReplyToName         string `json:"reply_to_name"`
	ReplyTo             string `json:"reply_to"`
	ResetURLLifetime    int    `json:"reset_url_lifetime"`
	SuccessRedirect     string `json:"success_redirect"`
	ErrorRedirect       string `json:"error_redirect"`
	EmailTextURL        string `json:"email_text_url"`
	EmailHTMLURL        string `json:"email_html_url"`
	ResetHTMLURL        string `json:"reset_html_url"`
	ResetSuccessHTMLURL string `json:"reset_success_html_url"`
	ResetErrorHTMLURL   string `json:"reset_error_html_url"`
}

type NewWelcomeEmailConfiguration struct {
	Enabled     bool   `json:"enabled"`
	URLPrefix   string `json:"url_prefix"`
	SenderName  string `json:"sender_name"`
	Sender      string `json:"sender"`
	Subject     string `json:"subject"`
	ReplyToName string `json:"reply_to_name"`
	ReplyTo     string `json:"reply_to"`
	TextURL     string `json:"text_url"`
	HTMLURL     string `json:"html_url"`
}

type NewSSOConfiguration struct {
	URLPrefix            string                     `json:"url_prefix"`
	JSSDKCDNURL          string                     `json:"js_sdk_cdn_url"`
	StateJWTSecret       string                     `json:"state_jwt_secret"`
	AutoLinkProviderKeys []string                   `json:"auto_link_provider_keys"`
	AllowedCallbackURLs  []string                   `json:"allowed_callback_urls"`
	Providers            []SSOProviderConfiguration `json:"providers"`
}

func (s *NewSSOConfiguration) APIEndpoint() string {
	// URLPrefix can't be seen as skygear endpoint.
	// Consider URLPrefix = http://localhost:3001/auth
	// and skygear SDK use is as base endpint URL (in iframe_html and auth_handler_html).
	// And then, SDK may generate wrong action path base on this wrong endpoint (http://localhost:3001/auth).
	// So, this function will remote path part of URLPrefix
	u, err := url.Parse(s.URLPrefix)
	if err != nil {
		return s.URLPrefix
	}
	u.Path = ""
	return u.String()
}

type SSOProviderConfiguration struct {
	Name         string `json:"name"`
	ClientID     string `json:"client_id"`
	ClientSecret string `json:"client_secret"`
	Scope        string `json:"scope"`
}

type UserVerificationConfiguration struct {
	URLPrefix        string                             `json:"url_prefix"`
	AutoUpdate       bool                               `json:"auto_update"`
	AutoSendOnSignup bool                               `json:"auto_send_on_signup"`
	AutoSendOnUpdate bool                               `json:"auto_send_on_update"`
	Required         bool                               `json:"required"`
	Criteria         string                             `json:"criteria"`
	ErrorRedirect    string                             `json:"error_redirect"`
	ErrorHTMLURL     string                             `json:"error_html_url"`
	Keys             []UserVerificationKeyConfiguration `json:"keys"`
}

func (c *UserVerificationConfiguration) ConfigForKey(key string) (UserVerificationKeyConfiguration, bool) {
	for _, keyConfig := range c.Keys {
		if keyConfig.Key == key {
			return keyConfig, true
		}
	}
	return UserVerificationKeyConfiguration{}, false
}

type UserVerificationKeyConfiguration struct {
	Key             string                                `json:"key"`
	CodeFormat      string                                `json:"code_format"`
	Expiry          int64                                 `json:"expiry"`
	SuccessRedirect string                                `json:"success_redirect"`
	SuccessHTMLURL  string                                `json:"success_html_url"`
	ErrorRedirect   string                                `json:"error_redirect"`
	ErrorHTMLURL    string                                `json:"error_html_url"`
	Provider        string                                `json:"provider"`
	ProviderConfig  UserVerificationProviderConfiguration `json:"provider_config"`
}

type UserVerificationProviderConfiguration struct {
	Subject     string `json:"subject"`
	Sender      string `json:"sender"`
	SenderName  string `json:"sender_name"`
	ReplyTo     string `json:"reply_to"`
	ReplyToName string `json:"reply_to_name"`
	TextURL     string `json:"text_url"`
	HTMLURL     string `json:"html_url"`
}
