...
func (c *End2End) QuerySQLSelectAudit(appID string, rawSQL string) (jsonArrString string, err error) {
	cfg, err := LoadConfigFromEnv()
	if err != nil {
		return "", err
	}

	db := openDB(cfg.AuditDatabase.DatabaseURL, cfg.AuditDatabase.DatabaseSchema)

	vars := map[string]interface{}{
		"AppID": appID,
	}

	tmpl, err := ParseSQLTemplate("sql-select-audit", rawSQL)
	if err != nil {
