...
docker_nginx_create_directories() {
	...
	# I do not know why.
	sudo rm /usr/share/nginx/modules
...
docker_nginx_render_server_block() {
	...
	*)
	...
	case "$scheme" in
	http)
		sed -i -E "s,__AUTHGEAR_HTTP_REDIRECT_${1}__,\"\"," /etc/nginx/nginx.conf
		;;
	https)
		sed -i -E "s,__AUTHGEAR_HTTP_REDIRECT_${1}__,y," /etc/nginx/nginx.conf
		;;
	*)
		;;
	esac
