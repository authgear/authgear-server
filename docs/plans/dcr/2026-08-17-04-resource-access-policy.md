...
> **Correction (post-launch review):** the "gating on third-party alone would also reject a resource-bound JWT, which the spec does not ask for" reasoning below was revisited and reversed, to avoid audience confusion: `/resolve` has no notion of "resource" and never exposes a token's `aud` to its caller, so accepting a third-party client's resource-bound JWT here would give that client access to every *other* resource `/resolve` protects. The gate actually implemented (`pkg/resolver/handler/resolve.go`) rejects **every** third-party client access token — opaque or JWT — not only the opaque one; only `client.IsFirstParty()` matters, `accessTokenIsJWT` does not. See `docs/specs/client.md`'s "Access Token Behavior by Client Kind" table and `access-token-audience-binding.md`'s "Authgear's decision" section for the current, corrected rule. The rest of this section is preserved as it was written, for history.
...
10. `/resolve` and `/oauth2/userinfo` matrix (§5.6, §8.3) — four cases, the last two being the regressions these tests exist to catch. *(Corrected post-launch: `/resolve` for a third-party client's resource-bound JWT was originally specified as succeeding below, per §5.6's now-reversed reasoning; the actual, corrected rule is that it must also be `invalid`, same as the opaque case.)*
    - third-party client, **opaque** token → `/resolve` invalid; `/oauth2/userinfo` **succeeds**.
    - third-party client, **resource-bound JWT** → `/resolve` **invalid** too (`client.IsFirstParty()` gates both token shapes, not opacity alone); `/oauth2/userinfo` still succeeds (must accept an `aud` that lacks the project endpoint).
...
- `/resolve` rejects every third-party client access token, opaque or resource-bound JWT (§5.6, corrected post-launch — see that section's correction note). Gating on opacity alone breaks every default first-party project, so `client.IsFirstParty()` is the only thing that matters; token shape is not. The rejection lives in `pkg/resolver`, not the shared session resolver, because the same opaque token must keep working at `/oauth2/userinfo`.
