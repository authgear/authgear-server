...
- An existing end-user with user ID `johndoe`.
...
### Use-cases: Authentication of end-user and authorization of end-user
...
### Use-cases: Authentication of client and authorization of client
...
- Determine whether `client_id` is authorized to access `scope` of `resource`, using some knowledge.
...
### Use-cases: handling of `access_token` in resources and its implication
...
- Verify if the `access_token` is signed by one of the JWK in `jwks_uri`.
- Check `aud`
  - If `aud` includes `https://auth.myapp.com`, then use RBAC to determine access.
    - This means check if `roles` is sufficient.
  - if `aud` includes `https://onlinestore.myapp.com`, then use `scope` to determine access.
    - This means check if `scope` is sufficient.
...
### Discussion: About the handling of `access_token` in resources
...
### Discussion: The `resource` parameter

To support M2M, we need to introduce Resource and its associated scopes.
...
we make scopes local to a specific resource.
...
### Discussion: Reserved value of scope
...
In addition to the well-known scopes, we also need to disallow creating scope that collide with Authgear's scope.
...
This section describes the protocol-level details.
