...
sequenceDiagram
    ...
    app1->>+Authgear: [1]GET https://auth.example.com/oauth2/authorize<br/>client_id=1<br/>scope=x_device_browser_session_token<br/>redirect_uri=app://redirect
...
Step [1] to [4] are the standard OpenID Connect authorization_code flow with the following extensions. In step [1], the `x_device_browser_session_token` is specified signifying that the client is requesting a `device_secret` to be returned when the code is exchanged for tokens.
...
Same as the [Native SSO](https://openid.net/specs/openid-connect-native-sso-1_0.html#name-native-app-authorization-ex), except that the new scope value supported is `x_device_browser_session_token`.

## Token Exchange Profile for App Initiated SSO to Web

This section profiles the OAuth2 Token Exchange [RFC8693](https://datatracker.ietf.org/doc/html/rfc8693) spec and describes the processing rules used to exchange a previous authentication for a device browser session token, which can then be used to exchange for a browser session of another Web App.
...
When a mobile app wants to perform App-initiated SSO to Web, it makes a request to the token endpoint to obtain a device browser session token with the following parameters:
...
- `requested_token_type`: Required. The value must be `urn:authgear:params:oauth:token-type:device-browser-session-token`.
...
- `access_token`: Required. The device browser session token which can then be used to exchange for a browser cookie. It is named as `access_token` for historical reasons as mentioned in [RFC8963](https://datatracker.ietf.org/doc/html/rfc8693#section-2.2.1).
- `issued_token_type`: Required. The value must be `urn:authgear:params:oauth:token-type:device-browser-session-token`.
- `token_type`: Required. The value is `Bearer` if DPoP header is not sent in the request, else the value is `DPoP`. See the [Binding Tokens To Device](#binding-tokens-to-device) for details on DPoP.
...
- `response_type`: Required. Must be one of the following values:
  - `app_initiated_sso_to_web session`: A long lived token will be set in the cookie, which shares the same lifetime of the refresh_token. However, it will not live longer than an IDP session.
  - `app_initiated_sso_to_web token`: A short lived token will be set in the cookie, which shares the same lifetime of an access_token.
...
- `authenticate()` will accept a new option `isAppInitiatedSSOToWebEnabled: boolean`. When `true`, the `x_device_browser_session_token` scope will be added to the authorize request.
- `makeAppInitiatedSSOToWebURL(): URL` will be added.
  - Parameters:
    - `clientID: string`: The client ID of the web app.
  ...
  - Error will be thrown if the current session does not support sso to web. i.e. x_device_browser_session_token does not exist in scope.
...
- Steps:
  ...
  1. In the mobile app, call `authenticate()` with `isAppInitiatedSSOToWebEnabled=true, clientID=CLIENT_ID_OF_WEB`. The mobile app is in authenticated state after user completed the login flow.
  2. In the mobile app, call `makeAppInitiatedSSOToWebURL()` with `redirect_uri=https://marketing-site.example.com`. An URL is returned from the method.
...
- Steps:
  ...
  1. In the mobile app, call `authenticate()` with `isAppInitiatedSSOToWebEnabled=true,isSSOEnabled=true, clientID=CLIENT_ID_OF_WEB`. The mobile app is in authenticated state after user completed the login flow.
  2. In the mobile app, call `makeAppInitiatedSSOToWebURL()` with `redirect_uri=https://spa.example.com`. An URL is returned from the method.
