...
- `authentication_flow`: The type of the authentication flow which triggers this event. `null` if event not triggered by authentication flow.
  - `type`: The type of the authentication flow. Possible values are `login`, `signup`, `reauth`, `promote`, `account_recovery`.
...
```json5
{
  "payload": {
    ...
    "identified_identities": [ /* The identified identities during the authentication */
      {
        "id": "8f84ed75-5c8b-45c1-b657-b0c65ac3affe",
        "claims": {
          "email": "user@example.com",
