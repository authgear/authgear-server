...
- baggage: `authgear_sdk_user_id={user_id}, authgear_sdk_device_id={device_id}`
  - authgear_sdk_user_id: The Authgear User ID, if the user is authenticated.
  - authgear_sdk_device_id: A unique UUID identifying the device. It will be generated on the first time `authgear.configure()` is called.

### Trace Context over HTTP Query Parameters

The Trace Context may also be propagated through HTTP query parameters, specifically for internal redirects where headers might be lost.

This is neccessary for endpoints like authorization endpoint to propagate the trace context, because these endpoints are opened by browsers.
