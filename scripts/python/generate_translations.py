...
def find_missing_keys(default_translation, current_translation):
    ...
    missing_keys = [
        ...
        if key not in current_translation and not key_is_reserved(key)
    ]
    return collections.OrderedDict(
        (key, default_translation[key]) for key in missing_keys
    )


def chunk_messages(
    messages: dict[str, str | dict[str, str]], chunk_size: int
) -> list[dict[str, str | dict[str, str]]]:
    keys = list(messages.keys())
    return [
        collections.OrderedDict((k, messages[k]) for k in keys[i : i + chunk_size])
...
def auto_translate(messages: dict[str, str | dict[str, str]], locale, chunk_size):
    ...
    for idx, chunk in enumerate(chunks):
        logging.info(
            f"{locale} | Starting translation of chunk {idx + 1}/{len(chunks)}"
        )
    ...
    """
        ...
        logging.info(
            f"{locale} | Finished translation of chunk {idx + 1}/{len(chunks)}"
        )
...
def save_translation_file(
    locale, default_translation, updated_translation, filepath, cldr_localnames_path
):
    translation = apply_cldr_territories(
        locale, updated_translation, cldr_localnames_path
    )
    translation = fix_translation_json(updated_translation)
    translation = collections.OrderedDict(
        (k, translation[k]) for k in default_translation.keys() if k in translation
    )
...
def update_translation(
    locale: str,
    default_translation_file: str,
    locale_translation_file: str,
    chunk_size,
    cldr_localnames_path,
):
    ...
    logging.info(
        f"{locale} | Found {len(missing_keys)} missing keys in {locale_translation_file}."
    )

    for translated_messages in auto_translate(
        messages=missing_keys, locale=locale, chunk_size=chunk_size
    ):
...
def make_update_locale_fn(cldr_localnames_path, chunk_size):
    def update_locale(locale):
        try:
            locale_info = LOCALE_DICT[locale]
            logging.info(
                f'{locale} | Starting translation for {locale} ({locale_info["name"]})'
            )
            ...
            logging.info(
                f'{locale} | Finished translation for {locale} ({locale_info["name"]})'
            )
