import { deriveOAuthProviderDisabled } from "./oauthProviders";
import {
  OAuthSSOProviderConfig,
  SSOProviderFormSecretViewModel,
} from "../types";

describe("deriveOAuthProviderDisabled", () => {
  const createConfig = (
  ...
  describe("Apple provider", () => {
    it("should be disabled when missing required fields", () => {
      const config = createConfig("apple");
      const secret = createSecret();
      expect(deriveOAuthProviderDisabled(config, secret)).toBe(true);
    });

    it("should be enabled when all required fields are present", () => {
      const config = createConfig("apple", {
        client_id: "client-id",
        key_id: "key-id",
        team_id: "team-id",
      ...
      expect(deriveOAuthProviderDisabled(config, secret)).toBe(false);
    });
  });
