...
const iosMachineMapping: { [name: string]: string } = {
...
export function formatDeviceInfo(deviceInfo: any): string | undefined {
  const android = deviceInfo["android"];
  if (android) {
    return formatAndroid(android);
  }

  const ios = deviceInfo["ios"];
  if (ios) {
...
export function formatAndroid(deviceInfo: any): string | undefined {
  if (typeof deviceInfo !== "object") return undefined;

  const build = deviceInfo["Build"];
  if (typeof build !== "object") return undefined;

  const manufacturer = build["MANUFACTURER"];
  if (typeof manufacturer !== "string") return undefined;
...
export function formatIOS(deviceInfo: any): string | undefined {
  if (typeof deviceInfo !== "object") return undefined;

  const uname = deviceInfo["uname"];
  if (typeof uname !== "object") return undefined;

  const machine = uname["machine"];
  if (typeof machine !== "string") return undefined;
