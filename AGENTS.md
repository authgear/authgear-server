...
| Project/feature config or secrets (`authgear.yaml`/`authgear.features.yaml`/`authgear.secrets.yaml`) | `pkg/lib/config/*.go` — **see `update-configs` skill** (feature config merge must be field-level; every surface has a required testdata fixture) |
| E2E tests | `e2e/tests/` |
